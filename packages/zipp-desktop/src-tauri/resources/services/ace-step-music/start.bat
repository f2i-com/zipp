...
if not exist "venv\Scripts\python.exe" (
    ...
    REM Check if acestep is installed, install if missing (use venv python directly)
    venv\Scripts\python.exe -c "import acestep" 2>nul
    if !errorlevel! neq 0 (
        echo [Setup] ACE-Step module not found, installing...
        venv\Scripts\pip.exe install git+https://github.com/ace-step/ACE-Step.git
    )
)
